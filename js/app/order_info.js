...
require(['zepto','loading','token','dialog','config','lazyload','base','homenav','fastclick','weixin'],function(zepto,loading,token,dialog,config,lazyload,base,homenav,fastclick,weixin) {
    ...
    var OI = {
            ...
            $.ajax({
                ...
                success : function(res){
                    if(res.errcode == 0){
                        ...
                        if(res.detail.my.addressPics && res.detail.my.addressPics.length){
                            $('.j_wraper').on('click','.j_address_img',function(){
                                weixin.previewImage($(this).attr('data-view'),res.detail.my.addressPics);
                            });
                        }
                        //homenav.init();
                        lazyload.init();
        ...
        createSuccessOrder:function(detail){
            ...
            //    +'<h2>有任何问题请使用如下方式联系客服</h2>'
            //    +'<p>1.在订阅号首页，点击左下角输入图标直接回复</p>'
            //    +'<p>2.请拨打24小时客服热线400-888-999，有中文客服MM为您解答</p>'
            //    +'<p>3.请添加客服MM微信号：XXXXXX  或直接扫下面二维码添加</p>'
            //    +'<div class="img-box">'
        ...
        createGoodsHtm : function(order_info){
            ...
            $.each(goods,function(i,item){
                ...
                _htm += '<a href="good_detail.html?goodid='+item.id+'"><li class="clearfix">'
